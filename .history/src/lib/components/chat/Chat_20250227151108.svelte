<script lang="ts">
	...
	import MessageInputTools from '$lib/components/chat/MessageInputTools.svelte';
	import MessageInputSelf from '$lib/components/chat/MessageInputSelf.svelte';
	...
	const createMessagesList = async(responseMessageId) => {
	...
	const submitPrompt = async (userPrompt, { _raw = false } = {}) => {
		...
		saveSessionSelectedModels();
		_responses = await sendPrompt(history,userPrompt, userMessageId, { newChat: true });
			...
			selectedModelIds.map(async (modelId, _modelIdx) => {
				...
				if (model) {
					...
					if (hasImages && !(model.info?.meta?.capabilities?.vision ?? true)) {
						toast.error(
							$i18n.t('APP {{modelName}} is not vision capable', {
								modelName: model.name ?? model.id
							})
						);
					}
					...
					toast.error($i18n.t(`APP {{modelId}} not found`, { modelId }));
				}
			})
		);
...
console.log('handleSubmit', type, data);
        if (type === 'form') {
            // 处理表单提交
            await submitPrompt(JSON.stringify(data));
        } else if (type === 'message') {
            // 处理普通会话提交
            await tick();
            submitPrompt(
	...
	$: currentModel = $models.find((m) => m.id === selectedModels[0]);
			...
			<Pane defaultSize={50} class="h-full flex w-full relative">
				...
				<div class="flex flex-col flex-auto z-10 w-full @container">
					{#if $settings?.landingPageMode === 'chat' || createMessagesList(history, history.currentId).length > 0}
						...
						<div class=" pb-[1rem]">
						{#if currentModel.info?.meta?.capabilities?.uploader ?? true}
							<MessageInput
								{history}
								{selectedModels}
								bind:files
