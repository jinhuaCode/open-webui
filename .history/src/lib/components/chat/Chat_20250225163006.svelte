...
=======
				...
				// await goto(`/g/?models=${encodeURIComponent('1')}`);
			}
		})();
	}
>>>>>>> dfef03c8e (同步远程)
...
=======
							$i18n.t('GPT {{modelName}} is not vision capable', {
>>>>>>> dfef03c8e (同步远程)
								modelName: model.name ?? model.id
							})
						);
...
=======
					...
					toast.error($i18n.t(`GPT {{modelId}} not found`, { modelId }));
>>>>>>> dfef03c8e (同步远程)
				}
			})
		);
...
=======
							{:else}
						<!-- {:else if selectedModels[0] === "PersonalAssistant" || selectedModels[0] === "flowchart"} -->
							<MessageInputTools
>>>>>>> dfef03c8e (同步远程)
								{history}
								{selectedModels}
