<script lang="ts">
	...
	import { getModels ,getTest} from '$lib/apis';
	...
	let models = [];
	let categories = [
    { id: 'recommended', label: '日常办公工具' },
    { id: 'work', label: '集团通用管理工具' },
    { id: 'life', label: '集团经营&风险管理工具' }
  ];
	let filteredModels = [];
...
// 根据 searchValue（搜索值）对 models（模型列表）进行过滤，然后将过滤后的结果赋值给 filteredModels
  ...
  if (model.id >= 1 && model.id <= 10) {
    return 'recommended';
  } else if (model.id >= 11 && model.id <= 20) {
    return 'work';
  } else if (model.id >= 21 && model.id <= 30) {
    return 'life';
  } else {
    return 'other'; // 默认分类
  }
  ...
  function toggleSidebarState(modelId, isShow) {
	...
	const hideModelHandler = async (model) => {
		...
		if (res) {
			toast.success(
				$i18n.t(`Apps {{name}} is now {{status}}`, {
					name: info.id,
	...
	onMount(async () => {
		models = await getWorkspaceModels(localStorage.token);
		console.log(getTest(localStorage.token));
...
{#if loaded}
	...
	<div class="flex flex-col gap-1 my-1.5">
		<div class="flex justify-center items-center">
			<div class="flex items-center text-3xl font-medium px-10.5">
				{$i18n.t('Apps')}
	...
	<nav class="sticky top-0 bg-white dark:bg-gray-800 shadow-sm z-10">
		<div class="container mx-auto px-10 py-2 flex gap-4">
		  {#each categories as category}
			<button
			  class="px-10 py-6 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition"
	...
	<div class=" my-2 mb-5 gap-2 grid lg:grid-cols-1 xl:grid-cols-1" id="model-list">
		{#each categories as category}
		<section id={category.id} class="scroll-mt-16">
		  <h2 class="text-2xl font-bold mb-4">{category.label}</h2>
		  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
		{#each categorizedModels[category.id] as model}
		{#if model.is_active}
		<!-- 顶部导航栏 -->
			...
			>
				<div class="flex gap-4 mt-0.5 mb-0.5">
					...
					<a
						...
						href={`/g/${encodeURIComponent(model.id)}`}
						...
						{:else}
							{#if model.user_id === $user?.id}
								<a
									class="self-center w-fit text-sm px-2 py-2 dark:text-gray-300 dark:hover:text-white hover:bg-black/5 dark:hover:bg-white/5 rounded-xl"
									type="button"
									href={`/Apps/models/edit?id=${encodeURIComponent(model.id)}`}
								>
									<svg
	...
	{#if $user?.role === 'admin'}
		<div class=" flex justify-end w-full mb-3">
			<div class="flex space-x-1">
				...
				>
					<div class=" self-center mr-2 font-medium line-clamp-1 text-xl">{$i18n.t('Import GPT')}</div>
				...
				>
					<div class=" self-center mr-2 font-medium line-clamp-1 text-xl">{$i18n.t('Export GPT')}</div>
...
{/if}
<div class="relative" style="padding-bottom: 50px;">

<div class="absolute bottom-4 right-4 " style="margin-top: 20px;">
	<img
