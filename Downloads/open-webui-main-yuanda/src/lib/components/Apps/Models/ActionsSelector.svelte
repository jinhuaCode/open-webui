...
<script lang="ts">
	...
	function removeSelectedAction(actionId) {
		_actions[FilterId].selected = false;
		selectedActionIds = selectedActionIds.filter((id) => id !== FilterId);
	}
</script>
<style>
	.dropdown {
...
<div dropdown {dropdownOpen ? 'open' : 'closed'}>
	<div class="flex w-full justify-between mb-1">
	...
	>
		{#if selectedActionIds.length > 0}
			<div >
				{#each selectedActionIds as FilterId}
					...
					type="button">
						...
						<span
							...
							on:click={() => removeSelectedAction(FilterId)}
						>
							X
						</span>
