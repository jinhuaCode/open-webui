<script lang="ts">
	...
	onMount(async () => {
		...
		const gptsmodels = await getWorkspaceModels(localStorage.token);

		showPinnedChat = localStorage?.showPinnedChat ? localStorage.showPinnedChat === 'true' : true;
		showInSidebar = localStorage?.showInSidebar ? localStorage.showInSidebar === 'true' : true;
	...
	>
		...
		<div class="px-1.5 flex justify-center text-gray-800 dark:text-gray-200">
			...
			>
				...
				<div class="flex self-center">
					<div class=" self-center font-medium text-sm font-primary">{$i18n.t('Masks')}</div>
				</div>
			</a>
		</div>
