...
from src.KD_loss import distillation_loss, patience_loss, contextual_loss
...
if DEBUG:
    ...
    argv = get_predefine_argv('glue', 'MNLI-mm', 'finetune_teacher')
    # 'bert-base-uncased',
    # # run simple fune-tuning *student* by uncommenting below cmd
    # argv = get_predefine_argv('glue',  'MNLI-mm', 'finetune_student')

    # run vanilla KD by uncommenting below cmd
    # argv = get_predefine_argv('glue', 'STS-B', 'kd')

    # run Patient Teacher by uncommenting below cmd
    # argv = get_predefine_argv('glue', 'STS-B', 'kd.cls')

    # run Patient Teacher by uncommenting below cmd
    # argv = get_predefine_argv('glue', 'STS-B', 'ckd')
    try:
...
if args.do_train:
    ...
    if args.teacher_prediction is not None and args.alpha > 0:
        ...
        elif args.kd_model == 'kd.cls':
            train_examples, train_dataloader, _ = get_task_dataloader(task_name, read_set, tokenizer, args,
                                                                      SequentialSampler,
                                                                      batch_size=args.train_batch_size,
                                                                      knowledge=teacher_predictions['pred_logit'],
                                                                      extra_knowledge=teacher_predictions['feature_maps'])
        elif args.kd_model == 'ckd':
            train_examples, train_dataloader, _ = get_task_dataloader(task_name, read_set, tokenizer, args,
                                                                      SequentialSampler,
                                                                      batch_size=args.train_batch_size,
                                                                      knowledge=teacher_predictions['pred_logit'])
        else:
            logger.info('kd_model is error!')
    ...
    # teacher_folder = f'kd_{task_name.upper()}_nlayer.12_lr.1e-05_T.10.0_alpha.0.0_beta.0.0_bs.32-run-1'
    # eval_log_file = os.path.join(f'/home/jingxu/project/PKD/data/outputs/KD/{task_name.upper()}/teacher_12layer', teacher_folder, 'eval_log.txt')
    # teacher_epoch = find_epoch(eval_log_file, task_name.upper()) - 1
    # teacher_information = teacher_folder.split('_')
    # teacher_n_layer = 12
    #
    # teacher_folder = os.path.join(f'/home/jingxu/project/PKD/data/outputs/KD/{task_name.upper()}/teacher_12layer', teacher_folder)
    #
    ...
    # # teacher_encoder_file = os.path.join(f'/home/jingxu/project/PKD/data/outputs/KD/CoLA/teacher_12layer', teacher_folder,f'CoLA_nlayer.12_lr.1e-05_T.10.0_alpha.0.0_beta.0.0_bs.32_e.{int(teacher_epoch)}.encoder.pkl')
    # # teacher_cls_file = os.path.join(f'/home/jingxu/project/PKD/data/outputs/KD/CoLA/teacher_12layer',
    # #                                     teacher_folder, f'CoLA_nlayer.12_lr.1e-05_T.10.0_alpha.0.0_beta.0.0_bs.32_e.{int(teacher_epoch)}.cls.pkl')
    # assert len(teacher_encoder_file) == 1 and len(
    #     teacher_cls_file) == 1, f'encoder/cls file error: {teacher_encoder_file}, {teacher_cls_file}'
    # teacher_encoder_file, teacher_cls_file = teacher_encoder_file[0], teacher_cls_file[0]
    #
    # config = BertConfig(os.path.join('/home/jingxu/project/PKD/data/models/pretrained/bert-base-uncased', 'bert_config.json'))
    # teacher_encoder_bert, teacher_classifier = init_model(task_name, args.kd_model, teacher_n_layer, config)
...
if args.kd_model.lower() in ['kd', 'kd.cls', 'ckd']:
    logger.info('using normal Knowledge Distillation')
    output_all_layers = args.kd_model.lower() in ['kd.cls', 'ckd']

    student_encoder, student_classifier = init_model(task_name, args.kd_model.lower(), args.student_hidden_layers, student_config)
...
if args.do_train:
    ...
    if args.task_name in ['CoLA']:
        print('epoch,mcc,loss', file=log_eval)
    elif args.task_name in ['QQP', 'MRPC']:
        print('epoch,acc,f1,loss', file=log_eval)
    elif args.task_name in ['STS-B']:
        print('epoch,Spear,loss', file=log_eval)
    else:
    ...
    for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
        ...
        for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
            ...
            if args.kd_model.lower() in['kd', 'kd.cls', 'ckd']:
                ...
                elif args.kd_model.lower() == 'ckd':
                    student_patience = torch.stack(full_output).transpose(0, 1)
                else:
            ...
            if args.kd_model == 'kd.cls':
                ...
                teacher_patience,_ = teacher_encoder_bert(input_ids, segment_ids, input_mask)
                teacher_patience = [f.detach().cpu().numpy() for f in teacher_patience]
                layer_index = [int(i) for i in args.fc_layer_idx.split(',')]
                teacher_patience = torch.stack(
                    [torch.FloatTensor(teacher_patience[int(i)]) for i in [0] + layer_index]).transpose(0, 1)

                wrdist_loss, wrang_loss, ltrdist_loss, ltrang_loss = contextual_loss(teacher_patience.cuda(), student_patience, input_mask)
                loss = kd_loss + wrdist_loss * 1.0 + wrang_loss * 1.0 + ltrdist_loss * 1.0 + ltrang_loss *1.0
            else:
        ...
        if args.task_name in ['CoLA']:
            ...
            print('{},{},{},{}'.format(epoch + 1, result['acc'], result['f1'], result['eval_loss']), file=log_eval)
        elif args.task_name in ['STS-B']:
            print('{},{},{}'.format(epoch + 1, result['spearmanr'], result['eval_loss']), file=log_eval)
        else:
