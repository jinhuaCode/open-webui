...
def eval_model_dataloader(encoder_bert, classifier, dataloader, device, detailed=False,
                          ...
                          verbose = False, output_mode='classification'):
    """
    ...
    # set loss function
    if detailed:
        feature_maps = [[] for _ in range(n_layer)]
        feature_all_maps = [[] for _ in range(n_layer)]
        predictions = []
        pooled_feat_maps = []
    ...
    for idx, batch in enumerate(tqdm(dataloader, desc="Iteration")):
        ...
        feat_all = encoder_bert.ckd(input_ids, segment_ids, input_mask)
        ...
        if detailed:
            ...
            if feat is None:
                ...
                for fm_all, f_all in zip(feature_all_maps, feat_all):
                    if need_reshape:
                        fm_all.append(f_all.contiguous().view(bs, -1).detach().cpu().numpy())
                    else:
                        fm_all.append(f_all.detach().cpu().numpy())
            if need_reshape:
    ...
    if detailed:
        ...
        feat_all_maps = [np.concatenate(t) for t in feature_all_maps] if len(feature_all_maps[0]) > 0 else None
        if n_layer == 24:
            return {'loss': loss,
                    'acc': acc,
                    ...
                    'feature_all_maps':[feat_all_maps[i] for i in [3, 7, 11, 15, 19]]}
        else:
            return {'loss': loss,
                    'acc': acc,
                    'pooled_feature_maps': np.concatenate(pooled_feat_maps),
                    ...
                    'feature_all_maps': feat_all_maps,}
...
def eval_model_dataloader_nli(task_name, eval_label_ids, encoder_bert, classifier, dataloader, kd_model, num_labels,
    ...
    for input_ids, input_mask, segment_ids, label_ids in dataloader:
        ...
        with torch.no_grad():
            ...
            if kd_model.lower() in['kd', 'kd.cls', 'ckd']:
