...
ALL_TASKS = ['MRPC', 'RTE', 'SST-2', 'MNLI', 'QQP', 'MNLI-mm', 'QNLI', 'CoLA', 'race-merge', 'STS-B']

if DEBUG:
    interested_task = 'MNLI-mm'.split(',')
    prediction_mode_input = 'teacher:train,dev'
    output_all_layers = 'kd'
    distill_type = 'kd'   # kd, pkd, ckd
...
for i, val in result_df.iterrows():
    ...
    if prediction_mode in ['benchmark']:
        ...
        elif distill_type == 'pkd':
            fname = os.path.join(output_dir, task, task + f'_patient_kd_teacher_{n_layer}layer_result_summary.pkl')
        elif distill_type == 'ckd':
            fname = os.path.join(output_dir, task, task + f'_contextual_kd_teacher_{n_layer}layer_result_summary.pkl')
        else:
            logger.info(f'distill method is error!')
