		...
		<div class=" flex flex-1 items-center w-full space-x-2">
			<div class="flex flex-1 items-center">
				...
				<input
					...
					placeholder={$i18n.t('Search GPTs')}
				/>
			</div>
						...
						{:else}
							{#if model.user_id === $user?.id}
								<a
									class="self-center w-fit text-sm px-2 py-2 dark:text-gray-300 dark:hover:text-white hover:bg-black/5 dark:hover:bg-white/5 rounded-xl"
									type="button"
									href={`/Masks/models/edit?id=${encodeURIComponent(model.id)}`}
								>
									<svg
	...
	{#if $user?.role === 'admin'}
		<div class=" flex justify-end w-full mb-3">
			<div class="flex space-x-1">
				...
				>
					<div class=" self-center mr-2 font-medium line-clamp-1 text-xl">{$i18n.t('Import GPTs')}</div>
				...
				>
					<div class=" self-center mr-2 font-medium line-clamp-1 text-xl">{$i18n.t('Export GPTs')}</div>
