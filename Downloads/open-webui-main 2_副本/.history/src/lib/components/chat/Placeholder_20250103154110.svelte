...
<div class="m-auto w-full max-w-6xl px-2 xl:px-20 translate-y-6 py-24 text-center">
		...
		<div class="w-full flex flex-col justify-center items-center">
			...
			<div
				class="text-base font-normal xl:translate-x-6 md:max-w-3xl w-full py-3 {atSelectedModel
					...
					: ''}"
			>
			{#if (models[selectedModelIdx]?.info?.meta?.capabilities?.uploader ?? false)}
	<MessageInputSelf
		{history}
		{selectedModels}
		bind:files
	...
	/>
{:else if (models[selectedModelIdx]?info?.meta?.capabilities?.uploader ?? false) && (models[selectedModelIdx]?.id === "PersonalAssistant" || models[selectedModelIdx]?.id === "flowchart")}
	<MessageInput
		{history}
		{selectedModels}
		bind:files
